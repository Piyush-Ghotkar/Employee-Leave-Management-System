...
//Code for tag and account in admin tbl
$tag="HOD";
if($department=="Computer Engineering"){
	$empid="HOD_CSE";
	$account=NULL;
$sql = "SELECT UserName from admin where UserName='$empid'";
...
if($query->rowCount() > 0){
	$empid=NULL;
	echo '<script>alert("Computer Engineering HOD already exists!\nPlease delete previous account to proceed.");</script>';
}}
}
...
if($query->rowCount() > 0){
	$empid=NULL;
	echo '<script>alert("Mechanical Engineering HOD already exists!\nPlease delete previous account to proceed.");</script>';
}}
}
...
if($query->rowCount() > 0){
	$empid=NULL;
	echo '<script>alert("Civil Engineering HOD already exists!\nPlease delete previous account to proceed.");</script>';
}}
}
...
if($query->rowCount() > 0){
	$empid=NULL;
	echo '<script>alert("Electrical Engineering HOD already exists!\nPlease delete previous account to proceed.");</script>';
}}
}
if($department=="First Year Engineering"){
	$empid="HOD_FE";
	$account="FE";
$sql = "SELECT UserName from admin where UserName='$empid'";
$query = $dbh -> prepare($sql);
...
if($query->rowCount() > 0){
	$empid=NULL;
	echo '<script>alert("First Year Engineering HOD already exists!\nPlease delete previous account to proceed.");</script>';
}}
}
...
if($query->rowCount() > 0){
	$empid=NULL;
	echo '<script>alert("Library HOD already exists!\nPlease delete previous account to proceed.");</script>';
}}
}
...
if($query->rowCount() > 0){
	$empid=NULL;
	echo '<script>alert("Workshop HOD already exists!\nPlease delete previous account to proceed.");</script>';
}}
}

if($empid!=NULL){
$sql1="INSERT INTO admin(id,UserName,Password,tag,account) VALUES(:BIOM,:empid,:password,:tag,:account)";
$query1 = $dbh->prepare($sql1);
$query1->bindParam(':BIOM',$BIOM,PDO::PARAM_STR);
$query1->bindParam(':empid',$empid,PDO::PARAM_STR);
$query1->bindParam(':password',$password,PDO::PARAM_STR);
$query1->bindParam(':tag',$tag,PDO::PARAM_STR);
  ...
  <?php include('includes/header.php');?>
   ...
   <main class="mn-inner">
                <div class="row">
                    ...
                    </div>&nbsp &nbsp &nbsp (Note: Available Leaves are Total Leaves offered to HOD)
...
<?php $sql = "SELECT Distinct Designation from tbldesignation ORDER BY designation";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
...
<div class="input-field col m6 s12">
<label for="T_CL">Enter Available Casual Leaves(CL)</label>
<input id="T_CL" name="T_CL" type="text" autocomplete="off" required>
 </div>
   
<div class="input-field col m6 s12">
<label for="CL">Enter Balance Casual Leaves(CL)</label>
<input  id="CL" name="CL" type="text" autocomplete="off" required>
</div>

<div class="input-field col m6 s12">
<label for="T_EL">Enter Available Earned Leaves(EL)</label>
<input id="T_EL" name="T_EL" type="text" autocomplete="off" required>
 </div>
   
<div class="input-field col m6 s12">
<label for="EL">Enter Balance Earned Leaves(EL)</label>
...
<div class="input-field col m6 s12">
<label for="T_FL">Enter Available Festival Leave(FL)</label>
<input id="T_FL" name="T_FL" type="text" autocomplete="off" required>
 </div>
   
<div class="input-field col m6 s12">
<label for="FL">Enter Balance Festival Leave(FL)</label>
